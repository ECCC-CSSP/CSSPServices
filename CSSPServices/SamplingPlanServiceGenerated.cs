...
namespace CSSPServices
{
    public partial class SamplingPlanService : BaseService
    {
        ...
        #region Constructors
        public SamplingPlanService(LanguageEnum LanguageRequest, IPrincipal User, DatabaseTypeEnum DatabaseType)
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext, ActionDBTypeEnum actionDBType)
        {
            ...
            //CreatorTVItemID (int) is required but no testing needed as it is automatically set to 0
            ...
            //DailyDuplicatePrecisionCriteria (float) is required but no testing needed as it is automatically set to 0.0f

            //IntertechDuplicatePrecisionCriteria (float) is required but no testing needed as it is automatically set to 0.0f

            //IncludeLaboratoryQAQC (bool) is required but no testing needed 

            if (samplingPlan.LastUpdateDate_UTC == null || samplingPlan.LastUpdateDate_UTC.Year < 1900 )
            {
            ...
            if (samplingPlan.Year < 1980 || samplingPlan.Year > 2050)
            {
                yield return new ValidationResult(string.Format(ServicesRes._ValueShouldBeBetween_And_, ModelsRes.SamplingPlanYear, "1980", "2050"), new[] { ModelsRes.SamplingPlanYear });
            }

            if (!string.IsNullOrWhiteSpace(samplingPlan.AccessCode) && (samplingPlan.AccessCode.Length < 3 || samplingPlan.AccessCode.Length > 15))
            {
                yield return new ValidationResult(string.Format(ServicesRes._LengthShouldBeBetween_And_, ModelsRes.SamplingPlanAccessCode, "3", "15"), new[] { ModelsRes.SamplingPlanAccessCode });
            }
