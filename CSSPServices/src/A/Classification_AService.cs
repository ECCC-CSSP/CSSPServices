...
namespace CSSPServices
{
    ...
    {
        #region Functions private Generated FillClassification_A
        private IQueryable<Classification_A> FillClassification_A()
        {
            Enums enums = new Enums(LanguageRequest);

            List<EnumIDAndText> ClassificationTypeEnumList = enums.GetEnumTextOrderedList(typeof(ClassificationTypeEnum));

             IQueryable<Classification_A> Classification_AQuery = (from c in db.Classifications
                let LastUpdateContactTVItemLanguage = (from cl in db.TVItemLanguages
                    where cl.TVItemID == c.LastUpdateContactTVItemID
                    && cl.Language == LanguageRequest
                    select cl).FirstOrDefault()
                    select new Classification_A
                    {
        ...
        #endregion Functions private Generated FillClassification_A
